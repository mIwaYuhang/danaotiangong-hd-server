...
def main():
    ...
    try:
        ...
        print('\n== 通天塔 ==')
        tower_daily = config.features.tower['daily_times']
        info = p.call('/Tower/GetTowerInfo')['Result']
        check('塔信息', info['Floor'] == 0 and info['RemainTowerChallengeTime'] == tower_daily, info)
        ...
        results = [p.call('/Tower/TowerBattle', type='1', ri='1', star='') for _ in range(tower_daily)]
        check('用满每日次数', all(r['State'] == 1 and 'Towerinfo' in r['Result'] for r in results), results[-1])
        ...
        check('超出每日次数被拒', p.call('/Tower/TowerBattle', type='1', ri='1', star='')['State'] == 99)
        info = p.call('/Tower/GetTowerInfo')['Result']
