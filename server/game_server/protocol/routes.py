...
def build_router(services, token_length: int) -> Router:
    ...
    r.role('/LevelGiftBag/BuyGrowup', s.activities.growup_buy)
    r.role('/LevelGiftBag/GetGrowupReward', s.activities.growup_claim, level=Integer())
    r.role('/Monthcard/GetMonthcardInfo', s.activities.month_card_info)
    r.role('/Monthcard/GetMonthcardReward', s.activities.month_card_claim)
    r.role('/Monthcard/GetWeekcardReward', s.activities.week_card_claim)
    r.role('/Monthcard/ChangeIngot', s.activities.change_ingot, point=Integer())
    r.role('/Recharge/RechargeLst', s.activities.recharge_list)
    r.role('/Recharge/firstreward', s.activities.first_recharge_info)
    r.role('/Recharge/GetFirstRechargeReward', s.activities.first_recharge_claim)
    r.role('/Recharge/GetOrderID', s.activities.get_order, money=Raw(default=''))
    r.role('/Recharge/GetPointOrderID', s.activities.get_point_order, money=Raw(default=''))

    # ---- 活动中心：财神到 / 幸运转盘 / 采灵芝 / 天女散花 / 坊市 / 分享 -------------------------
    r.role('/FortuneKing/FortuneKingInfo', s.promos.fortune_info)
    r.role('/FortuneKing/FortuneKingReward', s.promos.fortune_claim, ID=Integer(minimum=1))
    r.role('/Luckydisk/Luckydisk', s.promos.disk_info)
    r.role('/Luckydisk/SeekingTreasures', s.promos.disk_seek, index=Integer(minimum=0, required=False, default=0))
    r.role('/Luckydisk/Refresh', s.promos.disk_refresh)
    r.role('/Luckydisk/ExchangeTreasures', s.promos.disk_exchange, exchangeType=Integer(minimum=1))
    r.role('/Pickganoderma/Pickganoderma', s.promos.lingzhi_info)
    r.role('/Pickganoderma/Refresh', s.promos.lingzhi_refresh)
    r.role('/Pickganoderma/Call', s.promos.lingzhi_call, type=Integer(minimum=1))
    r.role('/Pickganoderma/Eat', s.promos.lingzhi_eat, heroID=Integer(minimum=1), type=Integer(minimum=1))
    r.role('/FairySendingFlowers/FlowersInfo', s.promos.sanhua_info)
    r.role('/FairySendingFlowers/Flowers', s.promos.sanhua_use,
           ingot=Raw(default='0'), gold=Raw(default='0'), energy=Raw(default='0'))
    r.role('/exchange/list', s.promos.exchange_list, activityid=Integer(minimum=1))
    r.role('/exchange/do', s.promos.exchange_do, exchangeid=Integer(minimum=1))
    r.role('/ActivityShare/complate', s.promos.share_complete)
    r.role('/Activegift/check', s.promos.gift_code, code=Raw(default=''))
    r.role('/promoter/add', s.promos.promoter_add, token=Raw(default=''))
    r.role('/promoter/reward', s.promos.promoter_rewards)
    r.role('/promoter/getreward', s.promos.promoter_claim, rewardid=Raw(default=''))

    # ---- 仙魔争霸（/Xm/*，客户端 CS* 场景） -----------------------------------------------
    r.role('/Xm/WorshipInfo', s.xianmo.worship_info)
    r.role('/Xm/Worship', s.xianmo.worship, beWorshipPlayerID=Integer(minimum=1), worshipType=Integer(minimum=1))
    r.role('/Xm/WorshipLog', s.xianmo.worship_log, beWorshipPlayerID=Integer(minimum=1))
    r.role('/Xm/RankInfo', s.xianmo.rank_info, type=Integer(minimum=1))
    r.role('/Xm/TeamInfo', s.xianmo.team_info, rankPlayerID=Integer(minimum=1))
    r.role('/Xm/BattleInfo', s.xianmo.battle_info, attackPlayerID=Integer(minimum=1),
           defendPlayerID=Integer(minimum=1), type=Integer(minimum=0, required=False, default=0))
    r.role('/Xm/BattlereportInfo', s.xianmo.battle_report, rankPlayerID=Integer(minimum=0, required=False, default=0),
           type=Integer(minimum=0, required=False, default=0), rankType=Integer(minimum=0, required=False, default=0))
    r.role('/Xm/Gamble', s.xianmo.gamble, beBetPlayerID=Integer(minimum=1), rank=Integer(minimum=1), gold=Integer(minimum=1))
    r.role('/Xm/GetGamble', s.xianmo.get_gamble, beBetPlayerID=Integer(minimum=1), rank=Integer(minimum=1))
    r.role('/Xm/RankReward', s.xianmo.rank_reward, type=Integer(minimum=1))
    r.role('/Xm/GetRankReward', s.xianmo.get_rank_reward)
